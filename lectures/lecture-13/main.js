...
function getSearchResults(term) {
    return fetch(getURLForSearch(term))
...
async function displaySearchResults() {
    ...
    const response = await getSearchResults(term);
    resultsElement.innerText = '';
    for (const result of response.results) {
        const li = createTrackDisplay(result.trackName, result.trackViewUrl,
        result.previewUrl, result.artworkUrl100);
